...
export default function RunPanel({ onRun, auditMode = false }) {
    ...
    <Panel title="Run Panel">
      <form className="stack" onSubmit={handleSubmit}>
        ...
        <label className="field">
          ...
          {auditMode ? (
            <input value="agent_starts" disabled />
          ) : (
        ...
        {form.start_mode === "agent_starts" ? (
          <>
            <label className="field">
              ...
              {auditMode ? (
                <input value="manual_or_fixture" disabled />
              ) : (
                <select
            ...
            {!auditMode && form.initial_message_source === "account" ? (
              <label className="field">
        ...
        <label className="field">
          <span>Scenario</span>
          {auditMode ? (
            <input value="outreach-screening-before-phone" disabled />
          ) : (
            <input name="scenario_id" value={form.scenario_id} onChange={updateField} />
          )}
        </label>
        {!auditMode && form.start_mode === "actor_starts" ? (
          <label className="field">
            <span>Actor</span>
        ...
        <label className="field">
          <span>Policy</span>
          {auditMode ? (
            <input value="production-policy-v1" disabled />
          ) : (
