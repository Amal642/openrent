...
testfix.arm_b3
--------------
ARM_B3: Embedding retrieval arm.

Query  = test source + error message + test name + pytest E-lines (NO function A source).
Corpus = all top-level functions in app/, embedded with text-embedding-3-small.
Top-5  = retrieved candidate helpers ranked by cosine similarity.
...
Comparison:
  ...
  ARM_B4 : see arm_b4.py (module import graph)
  ARM_B5 : see arm_b5.py (hybrid: import graph + embedding re-rank)

Pre-committed interpretation:
  >ARM_B2 recall@5 : embedding > BM25; lexical signal insufficient.
  <ARM_B2 recall@5 : lexical features carry more signal than semantics on this task.
  ARM_B  recall@5 : embedding closes the gap with oracle; structural retrieval redundant.

Usage (from openrent-agent/):
    python testfix/arm_b3.py [--attempts N] [--model MODEL] [--retrieval-only]
"""
...
def _build_prompt(
    ...
    if candidates:
        ...
        cand_section = (
            "\nTOP-5 CANDIDATE HELPER FUNCTIONS (retrieved by embedding similarity — "
            "one may contain the bug, or the bug may be in the entry-point itself):\n"
            + block
        )
...
def run_case_b3(
    ...
    candidates = emb_index.query(query_text, top_k=5, exclude_function=entry_func_name)
...
def main():
    ...
    parser.add_argument("--output", default="testfix/arm_b3_results.json")
    ...
    random_recall5 = 5 / emb_index.size if emb_index.size else 0
    print(f"  {emb_index.size} functions embedded. Random recall@5 baseline: {random_recall5:.1%}")
    ...
    print(f"\nARM_B3: {len(cases)} cases, model={args.model}"
    ...
    sep = "-" * 58
    print(f"\n{sep}")
    print(f"ARM_B3 RESULTS  model={args.model}  corpus={emb_index.size} fns  (embedding)")
    print(sep)
    print(f"  ARM_A  (no helpers)       : 0/7  =  0.0%")
    print(f"  ARM_B  (oracle call-graph): 7/7  = 100.0%")
    print(f"  ARM_B2 (BM25 top-5)       : 4/7  =  57.1%")
    if not args.retrieval_only:
        print(f"  ARM_B3 first-attempt      : {first_pass}/{total}  = {first_pass/total:.1%}")
        print(f"  ARM_B3 after-{args.attempts}-attempts   : {any_pass}/{total}  = {any_pass/total:.1%}")
        print(f"  median latency            : {median_lat} ms")
    print(f"\n  Corpus: {emb_index.size} fns  |  random recall@5 = {random_recall5:.1%}")
    print(f"\n  Retrieval recall (B in top-K of {emb_index.size}):")
    ...
    print(f"\n  Per-case (B=broken helper  rank=emb sim position  "
          f"{'pass=end-to-end' if not args.retrieval_only else 'retrieval-only'}):")
    for r in results:
        ret = r["retrieval"]
